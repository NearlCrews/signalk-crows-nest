...
panel uses the exact `signalk-nearlcrews-ui` package pinned in `package.json`
for its shell, themes, and shared controls, while the Signal K host supplies
React and React DOM at `^19.2.0`. The theme toggle offers a host-matching
default, a device-matching choice, and Light, Dark, and Night; the
configuration panel tour in the README describes what each automatic choice
follows. The panel build transpiles with Babel 8, whose React preset keeps
...
└── panel/                # Federated React configuration panel (bundled to public/)
    ...
    ├── endpoint-validation.ts # The usable-Overpass-endpoint checks
    ├── request-timeout.ts  # The panel-wide per-request timeout
    ├── source-names.ts     # The display name per source
