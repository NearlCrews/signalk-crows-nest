...
export interface PluginStatus {
   ...
   * added to the recent-errors list. The request's returned POI array carries
   * its own provenance, so overlapping calls cannot confuse a skip with a
   * successful fetch. The `reason` is retained on the source row as `lastSkip`
   * so the panel can explain why a quiet source is idle; a later successful or
   * failed request clears it. `transient` marks a deferral rather than a
   * deliberate gate (a list request that outran the aggregate's per-source
   * timeout and will be served from cache on the next refresh), which the panel
   * renders as waiting instead of idle.
   */
...
interface SourceState {
   ...
   * currently skipping. It persists on the snapshot so the panel can label a
   * quiet source with why, and is cleared by the next real
...
export function createPluginStatus (sources: ReadonlyArray<StatusSource>): PluginStatus {
  ...
  return {
    ...
    // surface in the recent-errors list. `reason` (the caller's skip
    // explanation, e.g. "outside US
