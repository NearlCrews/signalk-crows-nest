 ...
 * the route-corridor hazard scan, and the bridge air-draft check, grouped
 * under one collapsible section. These are source-agnostic: they alarm on
 * hazards from every enabled data source.
 *
 * Collapsed by default so the panel reads cleanly on open (a vessel that just
...
export default memo(function AlertsSection ({ state, dispatch }: Props): React.ReactElement {
  // Collapsed by default so the panel reads cleanly on open. The operator
